...
if ( ! function_exists( 'boston_business_customize_features_widget_fields' ) ) :
	 ...
	 */
	function boston_business_customize_features_widget_fields( $form_options, $widget ) {
...
add_filter( 'siteorigin_widgets_form_options_sow-features', 'boston_business_customize_features_widget_fields', 10, 2 );

if ( ! function_exists( 'boston_business_add_tab_in_builer_widgets_panel' ) ) :
	 ...
	 */
	function boston_business_add_tab_in_builer_widgets_panel( $tabs ) {
		$tabs['boston-business'] = array(
			'title'  => __( 'Boston Business Widgets', 'boston-business' ),
			'filter' => array(
				'groups' => array( 'boston-business' ),
			),
		);
		return $tabs;
	}
...
add_filter( 'siteorigin_panels_widget_dialog_tabs', 'boston_business_add_tab_in_builer_widgets_panel' );
...
if ( ! function_exists( 'boston_business_group_theme_widgets_in_builder' ) ) :
	 ...
	 */
	function boston_business_group_theme_widgets_in_builder( $widgets ) {
		...
		if ( isset( $GLOBALS['wp_widget_factory'] ) && ! empty( $GLOBALS['wp_widget_factory']->widgets ) ) {
			...
			foreach ( $all_widgets as $widget ) {
				if ( false !== strpos( $widget, 'Boston_Business_' ) ) {
					$widgets[ $widget ]['groups'] = array( 'boston-business' );
...
add_filter( 'siteorigin_panels_widgets', 'boston_business_group_theme_widgets_in_builder' );


if ( ! function_exists( 'boston_business_panels_row_style_attributes' ) ) :
	 ...
	 */
	function boston_business_panels_row_style_attributes( $attr, $style ) {
...
add_filter( 'siteorigin_panels_row_style_attributes', 'boston_business_panels_row_style_attributes', 10, 2 );

if ( ! function_exists( 'boston_business_customize_so_widgets_status' ) ) :
	 ...
	 */
	function boston_business_customize_so_widgets_status( $active ) {
