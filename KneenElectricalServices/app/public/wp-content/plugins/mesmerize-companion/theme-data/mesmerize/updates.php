...
function mesmerize_force_check_themes_update() {
    ...
    if ($transient) {
        //unset($transient->response['mesmerize-pro']);
...
function mesmerize_get_available_updates() {
    ...
    if ($themes && isset($themes['mesmerize-pro'])) {
        ...
        $needs_update['themes'] = array(array(
    ...
    if ($plugins) {
        foreach ($plugins as $file => $plugin) {
            if ($plugin->TextDomain == 'mesmerize-companion') {
                $needs_update['plugins'] = array(array(
...
$theme = wp_get_theme();
if ($theme && ($theme->template == "mesmerize-pro" || ($theme->parent() && $theme->parent()->template == "mesmerize-pro"))) {
