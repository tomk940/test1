    ...
    CP_Customizer.addModule(function (CP_Customizer) {
        ...
        function getColorComponent() {
            ...
            var $template = $(
                ...
                '       <div class="bg-picker">' +
                '            <h5 class="legend">' + 'Background Color' + '</h5>' +
        ...
        CP_Customizer.overlays.registerFixedOverlayOptions({
            'section': {
                'title': function (node) {
                    var label = $(node).attr('data-label') ? $(node).attr('data-label') + " [" + 'Section' + "]" : 'SECTION';
                    return label;
                },
                    ...
                    hover: [
                        function (event, overlay) {
                            ...
                            if ($(this).children().is('.gridContainer')) {
                                $changerItem.text('Make full width');
                                $changerItem.data('toMake', 'full');
                            } else {
                                $changerItem.text('Make Centered');
            ...
            'list': {
                ...
                'title': function (node) {
                    var label = $(node).closest('[data-label]').length ? $(node).closest('[data-label]').attr('data-label') + " [" + "List" + "]" : "List";
                    return label;
                },
                'toolbarTitle': function (node) {
                    var label = $(node).closest('[data-label]').length ? $(node).closest('[data-label]').attr('data-label') + " [" + "List" + "]" : "List";
                ...
                'items': [{
                    ...
                    'on_hover': [function (node) {
                        ...
                        for (var i = 0; i < cols.length; i++) {
                            var colNr = cols[i];
                            var itemData = {
                                'title': colNr + (colNr > 1 ? " " + "columns" : " " + "column"),
                                'name': colNr + "_columns",
                                'on_click': function (node) {
